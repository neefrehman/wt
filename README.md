...
- 📦 **Workspace-aware** — auto-detects workspaces to allow moving between them in a worktree
- 🔗 **Gitignored file syncing** — copies selected gitignored file categories from main worktree
...
```zsh
wt init                            # configure repo: editor, dependencies, sync categories

wt create                          # create worktree + branch with autogenerated name
wt create my-feature --open        # create named worktree + branch, open in editor
wt create --from develop           # create branch from ref
wt create --checkout feature/auth  # checkout feature/auth branch (do not create new branch)
wt create --cd --pr 42             # checkout GitHub PR, cd into worktree
wt create my-feature --stash       # move uncommitted changes to new worktree

wt open                            # open worktree, or open workspaces in current worktree
wt open my-feature --open          # open directly in editor
wt open my-feature --cd            # cd directly into worktree

wt rename old-name new-name        # rename worktree (and optionally its branch)
wt rename new-name                 # Rename current worktree (and optionally its branch)

wt sync                            # re-sync gitignored files from main worktree
wt delete                          # delete current worktree, or select from main
wt clean                           # bulk-delete worktrees with merged/gone branches
wt list                            # list worktrees with branches, themes, and status
```
...
4. **Workspace files** -- auto-detects `*.code-workspace` files to allow moving between workspaces in a worktree
...
`wt` does not support arbitrary post-creation hooks. If you need to run custom scripts after a worktree is created (e.g. setting up local tooling, creating symlinks, or running project-specific setup), you can use Git's built-in `post-checkout` hook which fires after `git worktree add`. See [this post on using git hooks when creating worktrees](https://mskelton.dev/bytes/using-git-hooks-when-creating-worktrees) for a walkthrough.
